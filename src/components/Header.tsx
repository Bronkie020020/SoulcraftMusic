...
export const Header: React.FC<HeaderProps> = ({
  ...
  return (
    <header className="sticky top-0 z-40 bg-black/95 backdrop-blur-xl border-b border-zinc-800 text-white transition-all pt-[env(safe-area-inset-top,0px)] shadow-md">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 py-2.5 sm:py-0 sm:h-20 flex flex-col sm:flex-row items-center justify-between gap-2.5 sm:gap-4">
        ...
        <div className="w-full sm:w-auto flex items-center justify-between sm:justify-start gap-3">
          <div className="flex items-center gap-3">
            <div
              ...
              className="relative flex items-center justify-center w-11 h-11 sm:w-13 sm:h-13 rounded-2xl bg-zinc-950 border border-amber-500/40 shadow-lg shadow-amber-500/10 group cursor-pointer shrink-0 overflow-hidden ring-1 ring-amber-400/20"
            >
              <img
                src="/logo.png"
                alt="Soulcraft Logo"
          ...
          <div className="flex items-center gap-1.5 sm:hidden">
            {onOpenQRCode && (
              <button
                ...
                className="p-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 text-yellow-400 text-xs font-bold transition-colors shadow-sm active:scale-95 min-h-[38px] min-w-[38px] flex items-center justify-center"
                title="QR Code"
              >
            ...
            <button
              ...
              className="px-2.5 py-1.5 rounded-xl bg-zinc-900 border border-zinc-700 text-xs font-bold text-white transition-colors shadow-sm active:scale-95 min-h-[38px] flex items-center gap-1"
            >
        ...
        <div className="w-full sm:w-auto flex items-center justify-center bg-zinc-900/90 border border-zinc-800 p-1 rounded-2xl sm:rounded-full shadow-inner">
          <button
            onClick={() => setActiveTab('downloader')}
            className={`flex-1 sm:flex-initial px-4 md:px-5 py-2 sm:py-2 rounded-xl sm:rounded-full text-xs md:text-sm font-bold transition-all flex items-center justify-center gap-2 min-h-[40px] ${
              activeTab === 'downloader'
                ? 'bg-yellow-400 text-black shadow-md font-black scale-[1.02]'
        ...
        <div className="hidden sm:flex items-center gap-2 sm:gap-3">
          ...
          {downloadCount > 0 && (
            <button
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-xl border text-xs cursor-pointer transition-all ${
                isOverLimit
                  ? 'bg-red-500/20 border-red-500/50 text-white font-extrabold shadow-sm animate-pulse'
                  : 'bg-zinc-900 border-zinc-700 text-white hover:bg-zinc-800'
              }`}
              title="Opslag beheren / Manage storage"
            >
              <HardDriveDownload className="w-4 h-4 text-yellow-400" />
              <span className="font-bold text-white">{downloadCount}</span>
              <span className="text-yellow-400 font-mono font-bold">({totalStorageMb.toFixed(1)} MB)</span>
            </button>
          )}
          ...
          {onOpenWindowsInstall && (
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-yellow-400 hover:bg-yellow-300 text-black text-xs font-black transition-colors shadow-sm cursor-pointer"
          ...
          {onOpenQRCode && (
            <button
              onClick={onOpenQRCode}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 text-yellow-400 text-xs font-bold transition-colors shadow-sm cursor-pointer"
          ...
          <button
            onClick={() => setLanguage(language === 'nl' ? 'en' : 'nl')}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-700 text-xs font-bold text-white transition-colors shadow-sm"
